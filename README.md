...
|--------|---------|
| `npm test` | Full suite |
| `npm run test:e2e` | Single full flow |
| `npm run test:api` | API-only |
| `npm run test:component` | Tests tagged `@component` |
| `npm run test:negative` | Only **`@negative`** error-path tests |
...
1. **Hosted SUT** — `use.baseURL` targets ParaBank
...
4. **Shared flows** — [`tests/e2e/support/parabank-flow.helpers.ts`](tests/e2e/support/parabank-flow.helpers.ts) keeps ParaBank UI/API flows in one place for the full e2e flow and component specs.
